...
import { ADMINJS_COMPOSITE_ID_PROPERTY, CompisiteIdProperty, } from './CompositeIdProperty.js';
export class Resource extends BaseResource {
    ...
    prepareProperties() {
        ...
        if (primaryKey && primaryKey.fields.length > 1) {
            const idProperty = new CompisiteIdProperty(primaryKey.fields.map((field) => fields.find((f) => f.name === field)), Object.keys(properties).length);
    ...
    prepareParams(params) {
        ...
        for (const property of this.properties()) {
            if (property === this.idProperty) {
                const primaryKeyValue = this.model.primaryKey.fields.reduce((pk, field) => {
